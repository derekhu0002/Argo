...
- W6 structural result completion begins only after W5 range selection. It must add same-version endpoint Elements for every returned ArchitectureRelationship, complete only selected View objects with metadata/members/relationships/endpoints, prevent overlapping-View cascade by shared membership alone, assign exactly one ordered `firstInclusionReason` per returned object, preserve supplementary reasons without overwriting the first reason, and expose canonical graph, semantic content/index, alignment, purpose, and policy evidence.
