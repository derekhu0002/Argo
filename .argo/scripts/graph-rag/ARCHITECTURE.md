...
`createSystemMetadataCommandAdapter({ repositoryRoot, executeMetadataCommand })` in `systemMetadataCommandAdapter.js` is the only production boundary allowed to import Node `child_process`. Production configuration composition creates it internally and injects its four capability methods into the resolver: `isSecretFileIgnored()`, `isSecretFileTracked()`, `readCurrentIdentity()`, and `readSecretFileAcl()`. The methods accept no executable, argv, path, environment, stdin, or secret values. The optional executor seam exists only for the frozen recording probe; the public resolver never accepts a caller-provided metadata adapter.
...
- `systemMetadataCommandAdapter.js` is the sole system-process exception. It uses `spawnSync` with `shell: false`, an exact repository cwd, UTF-8 output, Windows-hidden execution, and a sanitized environment limited to PATH/PATHEXT/SystemRoot/WINDIR. Its complete command allowlist is `git check-ignore --quiet -- .argo/.env`, `git ls-files --error-unmatch -- .argo/.env`, `whoami`, and `icacls <exact canonical repository .argo/.env path>`.
