...
`resolveApprovedLiveConfiguration({ repositoryRoot, adapters })` is the single public configuration/preflight boundary. Frozen fixtures call this exact function with temporary files and injected filesystem, git, ACL, structured-source, and forbidden-side-effect adapters. `source.readProcessKey(key)` returns frozen `{ value, trace }`; `source.readFileEntries(path)` returns a frozen array of frozen `{ key, value, trace }` records. Each trace is generated during the actual adapter read and contains exactly source kind, path, key, operation, and frozen alias chain. `source.isIssuedTrace(trace)` is the adapter trust capability backed by trace object identity; cloned, mutable, caller-created, malformed, or unvalidated traces are rejected. Callers cannot submit a provenance verdict. Accepted results expose the complete normalized approved configuration plus sanitized per-key source attribution; rejected results expose only stable categories.
...
- The resolver validates every returned trace before consuming its value: the trace and alias chain are frozen, the adapter confirms object identity through `isIssuedTrace`, source/path/key/operation match the requested read, and direct approved reads have a one-element alias chain equal to the requested key. The same value with a CLI/literal source kind, fallback/indirect operation, or multi-hop alias remains prohibited.
