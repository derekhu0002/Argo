...
- The MCP gateway exposes `backfillSystemArchitectureSemanticProjection` as the explicit operator control point and delegates to `runtime.runSemanticBackfill(request)`. The shipped JSON-RPC `tools/call` path must construct and use the production runtime composition root without caller- or Harness-injected `productionGraphRagRuntime`/`productionGraphRagDependencies`; deterministic acceptance launches the actual MCP process with external secrets absent and requires fail-closed `EXTERNAL_CREDENTIALS_REQUIRED` rather than `productionGraphRagRuntime.runSemanticBackfill is required`. Missing `explicitOptIn === true` fails `SP01_EXPLICIT_OPT_IN_REQUIRED`; a structural/canonical version mismatch fails `SP01_STRUCTURAL_VERSION_MISMATCH`; neither path may invoke provider or durable side effects. Default composition and structural mutation or sync must never trigger fake canonical mutation.
