...
- The MCP gateway exposes `backfillSystemArchitectureSemanticProjection` as the explicit operator control point and delegates to `runtime.runSemanticBackfill(request)`. Missing `explicitOptIn === true` fails `SP01_EXPLICIT_OPT_IN_REQUIRED`; a structural/canonical version mismatch fails `SP01_STRUCTURAL_VERSION_MISMATCH`; neither path may invoke provider or durable side effects. Structural mutation or structural sync must never become an implicit semantic trigger.
