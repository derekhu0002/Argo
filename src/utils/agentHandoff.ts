...
export function buildImplementationDesignHandoffPrompt(input: {
    ...
    const lines = [
        '请作为 Copilot 主 agent 完成以下工作：',
        `1. 分析范围仅限当前工作区 ${input.workspacePath}。必须先读取 design\\KG\\SystemArchitecture.json，再读取项目根目录下的 OVERALL_ARCHITECTUR.md（若存在）以及受影响目录下的 ARCHITECTURE.md 契约文件，再按需读取代码、测试、脚本、配置与文档。凡是能从仓库和工具结果确认的事实，不要向用户追问。`,
        ...
        '5. 本次产出的实现架构是直接落盘为代码仓中的实现架构本体：项目根目录下的 OVERALL_ARCHITECTUR.md、稳定实现元素目录下的 ARCHITECTURE.md、必要的目录/文件布局、显性测试入口、关键非显性测试与普通支撑测试护栏。',
        ...
        '8. implements 关系必须通过 OVERALL_ARCHITECTUR.md 与相关 ARCHITECTURE.md 契约显式声明。不是每个实现元素都必须直接连到意图元素。允许存在间接实现链路，例如实现元素 C implements 实现元素 B，而 B implements 意图元素 A；在这种情况下，C 通过实现链间接承载 A。你的任务是让这种实现链条可追踪、可解释，而不是强制所有节点都直接挂到意图层。',
        ...
        '   - 契约落盘结果：说明你已更新 OVERALL_ARCHITECTUR.md 与哪些 ARCHITECTURE.md，并概述关键规则、关键元素与局部契约',
