...
export function buildImplementationDesignHandoffPrompt(input: {
    ...
    const lines = [
        ...
        '4. 显性 testcase 的入口在本阶段完成后，预期应该能够被实际运行，并且在业务实现尚未补齐前允许且预期先以失败结果暴露缺口；不要把显性 testcase 设计成在实现缺失时也能伪通过。',
        ...
        '6. 显性 testcase 入口在本阶段完成后应被实际执行校验；若相关业务实现尚未完成，预期结果应是失败且失败原因可读，以便后续 /work 阶段围绕真实缺口修复，而不是让测试入口虚假通过。',
        ...
        '11. 除非用户明确要求，否则本次任务不要直接修改业务功能实现；重点是维护实现架构契约、显性 testcase 入口设计、关键非显性测试冻结与后续编码护栏，而不是直接进入业务编码。',
        '',
        '### Required Output',
        '   - 仓库已证实的事实与当前实现约束',
        ...
        '   - 显性 testcase 入口物理化结果：说明哪些显性 testcase 已有只读入口、哪些入口需要新建或补位、各自的关键断言如何落地、各自的控制点与观测点，以及这些入口如何交给后续编码阶段直接调用',
        '   - 显性 testcase 首次执行结果：说明哪些入口已经实际运行、当前是通过还是失败；若失败，失败是否符合“实现尚未补齐”的预期以及失败信息如何指向真实缺口',
