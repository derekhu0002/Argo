...
export function buildWorkAgentHandoffPrompt(input: {
    ...
    const lines = [
        ...
        '8. 只要涉及测试用例，无论是读取失败记录、补齐普通非显性测试，还是说明测试修复方案，都必须显性描述该测试的“控制点”和“观测点”：控制点是触发行为的入口、输入、前置布置或执行动作；观测点是被断言的外部可观察输出、状态、产物、日志、错误或副作用。缺少任一项都视为测试设计不完整。',
        '9. 修复完成后，执行失败测试记录中 `acceptanceCriteria` 指向的既有测试入口，直到这些失败全部通过；只要仍有失败，就继续修改、继续执行，不需要请示用户。',
        '10. 如新增或调整外部接口，必须同步更新项目根目录的 INTRODUCTION.md，确保对外说明与真实接口一致。',
        '11. 如果发现缺失显性测试入口、关键非显性测试契约错误、关键护栏失效且必须改写，或测试环境信息只能通过改写冻结资产才能补齐，请将其视为实现架构设计阶段缺口并明确回报，不要在编码阶段直接改写这些冻结资产。',
        '12. 完成后，请回复：',
    ...
    if (input.totalTestCases === 0) {
        ...
        lines.push(`13. 当前有 ${input.missingCriteriaCount} 个显性 testcase 缺少 acceptanceCriteria；请将其明确标记为实现架构设计阶段尚未物理化完成的契约缺口，不要在编码阶段直接补写或改写显性测试入口。`);
    }
...
export function buildImplementationDesignHandoffPrompt(input: {
    ...
    const lines = [
        ...
        '4. 本次产出必须直接落盘为代码仓中的实现架构本体：项目根目录下的 OVERALL_ARCHITECTURE.md、稳定实现元素目录下的 ARCHITECTURE.md、必要的目录/文件布局、显性测试入口、关键非显性测试与普通支撑测试护栏。',
        '5. 本次产出的实现架构必须保持高层稳定边界，不要退化成源码镜像或函数级设计。',
        '6. 对于意图架构中的显性 testcase，你除了建立追溯关系外，还必须为每条需要落地的显性 testcase 明确其单一测试入口如何物理化，使后续编码阶段可以“直接调用而不修改”。若仓库中尚不存在该入口，本阶段应负责设计并产出对应入口文件或明确其只读落点，而不是把这项责任下推给编码阶段。',
        '7. 所有测试用例设计都必须显性描述“控制点”和“观测点”。控制点是触发行为的入口、输入、前置布置或执行动作；观测点是被断言的外部可观察输出、状态、产物、日志、错误或副作用。无论是显性 testcase、关键非显性测试还是普通支撑测试，只要缺少控制点或观测点描述，都视为设计不完整，不能算交付完成。',
        '8. 非显性测试必须分层处理：',
        ...
        '9. OVERALL_ARCHITECTURE.md 与 ARCHITECTURE.md 的契约格式必须统一采用共享骨架，但根契约与元素契约承担不同字段职责。根级总入口由 OVERALL_ARCHITECTURE.md 唯一承载；子目录局部契约默认由 ARCHITECTURE.md 承载。ARCHITECTURE.md 可以引用 OVERALL_ARCHITECTURE.md，但不得重复定义根级规则。',
        ...
        '12. 除非用户明确要求，否则本次任务不要直接修改业务功能实现；重点是维护实现架构契约、显性 testcase 入口设计、关键非显性测试冻结与后续编码护栏，而不是直接进入业务编码。',
    ];
