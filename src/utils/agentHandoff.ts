...
export function buildWorkAgentHandoffPrompt(input: {
    ...
    const lines = [
        ...
        '   - 如果发现某条显性 testcase 缺少单一测试入口，或现有入口本身失效且需要改写，请将其视为实现架构设计阶段的缺口并明确回报；你可以补齐支撑性测试、夹具、数据、环境与业务实现，但不要在编码阶段改写显性测试基线文件',
...
export function buildImplementationDesignHandoffPrompt(input: {
    ...
    const lines = [
        ...
        '10. 对于意图架构中的显性 testcase，你除了在实现架构中建立追溯关系外，还必须为每条需要落地的显性 testcase 明确其单一测试入口如何物理化，使后续编码阶段可以“直接调用而不修改”。若仓库中尚不存在该入口，本阶段应负责设计并产出对应入口文件或明确其只读落点，而不是把这项责任下推给编码阶段。',
        '11. 非显性测试用例直接写入 `design/KG/ImplementationArchitecture.json`，作为实现架构模型的一部分，挂载到相应实现元素下的 `testcases` 字段中。它们是后续编码阶段的支撑性验证输入，但不属于意图架构中的显性 testcase 基线。优先使用 schema 已有 testcase 结构；必要时可在testcase的描述中补充这些测试所保护的实现决策、显性 testcase 或前置条件。测试也必须服从高层颗粒度原则：优先挂在稳定实现边界上，不要因为某个函数存在就为它单独造一个架构元素来挂测试。',
        '12. 这些非显性测试用例只需要明确说明它们直接支持的上游对象，例如某个实现模块、某项实现决策，或某条上一级支撑性测试用例；不要求每条都直接写出最终对应的显性 testcase，只要沿着追溯链能够分析出它最终支撑到哪条显性 testcase 即可。',
        '13. 按决策依赖顺序推进。先自己识别当前代码中的职责缠结、接口泄漏、shallow module 风险、不合理依赖方向以及实现承载缺口；然后只把真正高杠杆的架构决策提交给用户拍板。不要把可以通过仓库证据自己得出的结论丢给用户。若你发现自己开始按文件、类、函数或局部流程枚举实现元素，应立即回退并重新压缩成高层稳定边界。',
        '14. 输出必须使用中文，并严格包含以下内容：',
        '   - 仓库已证实的事实与当前实现约束',
        ...
        '15. 除非用户明确要求，否则本次任务不要直接修改业务代码；重点是维护实现架构模型、显性 testcase 入口设计与后续编码护栏，而不是直接进入编码。',
    ];

    if (input.extraContext) {
        lines.push(`16. 当前补充上下文：${input.extraContext}`);
    }
