...
export function buildTestDesignHandoffPrompt(input: {
    ...
    const lines = [
        ...
        '5. 除显性 testcase 外，你还可以设计非显性 testcase，并将其写入 #file:supporting-testcases.json。这里的“非显性 testcase”特指：不写入意图架构、不直接承担架构验收职责、但用于支撑实现、局部验证、环境校准和回归防退化的测试。',
        ...
        '10. 你必须同步维护 #file:supporting-testcases.json：对每条非显性 testcase 至少写明 `name`、`kind`、`role`、`verifies`、`supportsExplicitTestcase`、`targetIntentElementId`、`suggestedEntry`、`preconditions`、`keyAssertions`、`status`。其中 `role` 固定为 `supporting`。',
        ...
        '   - 回填计划：分别列出需要新增或更新到 #file:SystemArchitecture.json 的显性 testcase，以及需要新增或更新到 #file:supporting-testcases.json 的非显性 testcase',
        ...
        '14. 对于 #file:supporting-testcases.json 中的非显性 testcase，你可以直接新增或更新，因为它们属于测试设计阶段的支撑性产物；但不得借此绕过显性 testcase 的确认流程。',
        '15. 如果某个已有显性 testcase 已不再适配当前系统内容，先把其 `acceptanceCriteria` 置空，并在说明中记录需要重建的原因和原测试入口，再等待新的实现与测试入口落地。',
        '16. 本次任务不要求直接修改业务代码，但要求产出可执行的测试设计，并在用户确认后仅将验收测试、场景测试和子系统间集成测试作为意图架构的显性测试基线回填到 #file:SystemArchitecture.json；其他测试应写入 #file:supporting-testcases.json，作为编码阶段可消费的支撑实现。',
    ];
