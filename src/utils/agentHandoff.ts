...
export function buildTestDesignHandoffPrompt(input: {
    ...
    const lines = [
        ...
        '   - 测试用例草案：每条显性 testcase 至少写明名称、挂载元素、被验证职责、前置条件、输入/操作、关键断言、测试类型、建议入口、反作弊约束；支撑性测试只需说明它支撑哪条显性 testcase，不要求回填到意图架构',
        '   - 回填计划：仅列出需要新增或更新到 #file:SystemArchitecture.json 的验收测试、场景测试或子系统间集成测试 testcase',
        '10. 在新增、修改或删除任何 testcase 之前，必须先向用户逐项展示拟议变更、原因、影响范围和预期收益，并征求确认；未经确认，不得直接回填。若用户不同意，你必须继续收敛分歧，而不是强行写回。',
        '11. 一旦获得用户确认，回填到 #file:SystemArchitecture.json 的 testcase 对象至少必须包含：`name`、`description`、`Input`、`acceptanceCriteria`、`TestResults`。只有验收测试、场景测试或子系统间集成测试可以按此方式显性回填；其中 `description` 要写清测试目标、关键断言、前置条件以及是否依赖真实环境；`acceptanceCriteria` 必须指向单一测试入口，不得写成命令拼接。',
        '12. 如果某个已有 testcase 已不再适配当前系统内容，先把其 `acceptanceCriteria` 置空，并在说明中记录需要重建的原因和原测试入口，再等待新的实现与测试入口落地。',
        '13. 本次任务不要求直接修改业务代码，但要求产出可执行的测试设计，并在用户确认后仅将验收测试、场景测试和子系统间集成测试作为意图架构的显性测试基线回填到 #file:SystemArchitecture.json；其他测试留在测试代码层作为支撑实现。',
    ];
