...
export function buildWorkAgentHandoffPrompt(input: {
    ...
    const lines = [
        ...
        '3. 读取实现架构根契约：项目根目录下的 #file:OVERALL.md。若不存在该文件，请将其视为实现架构设计阶段缺口并明确回报。',
        '4. 根据失败记录、受影响路径与追溯链，继续读取相关实现元素目录下的 README.md 契约文件；这些 README 与 #file:OVERALL.md 一起构成当前实现架构约束。',
        '5. 以失败记录作为唯一待修复清单，并把 #file:OVERALL.md 与相关 README.md 契约作为实现与补测试的辅助约束，直接修改当前工作区代码，而不是只给建议。',
        '6. 任何代码修改都必须同时遵守意图架构图谱与实现架构契约中已经确定的原则、边界、依赖方向和实现映射，不能为了让测试通过而绕开这些约束，更不能引入新的架构违规。',
        '7. 如果当前代码与 #file:OVERALL.md 或相关 README.md 中声明的模块职责、接口边界、分层关系、实现链、测试护栏归属不一致，应优先把实现拉回既定架构，再补功能或补测试。',
        ...
        '   - 显性 testcase 与相关 README.md 中定义的关键/普通非显性测试必须通过追溯链保持一致：显性 testcase 的目标、挂载对象、入口、范围变化时，要同步检查受影响的实现元素及其下测试护栏；反过来，普通支撑测试变化也不得破坏上游显性基线与关键护栏的成立条件',
        '   - 如果发现某条显性 testcase 缺少单一测试入口、某条关键非显性测试本身契约错误，或现有入口/关键护栏失效且需要改写，请将其视为实现架构设计阶段的缺口并明确回报；你可以补齐支撑性测试、环境与业务实现，但不要在编码阶段改写这些冻结资产',
        '   - 需要补齐支撑性验证时，应优先把普通非显性测试写回到对应实现元素目录下的 tests/ 中，并遵守 #file:OVERALL.md 与局部 README 契约中声明的归属规则',
        ...
        '13. 完成后，请回复：',
        '   - 读取了哪些契约文件（OVERALL.md 与哪些 README.md）',
        '   - 修改了哪些代码',
        '   - 新增或更新了哪些内外部接口',
...
export function buildImplementationDesignHandoffPrompt(input: {
    ...
    const lines = [
        ...
        '4. 设计时必须显式应用并检查这些原则：整洁架构、SOLID、DEEP MODULE、渐进式披露、关注点分离、稳定抽象依赖方向。这里的“渐进式披露”不是要求输出迁移路线图，而是要求你的架构契约本身有层次性，优先通过根契约、局部契约、目录层级、README 追溯和测试护栏组织结构，让人类和 AI 都能逐层理解。',
        '5. 本次产出的实现架构是直接落盘为代码仓中的实现架构本体：项目根目录下的 OVERALL.md、稳定实现元素目录下的 README.md、必要的目录/文件布局、显性测试入口、关键非显性测试与普通支撑测试护栏。',
        ...
        '8. implements 关系必须通过 OVERALL.md 与相关 README.md 契约显式声明。不是每个实现元素都必须直接连到意图元素。允许存在间接实现链路，例如实现元素 C implements 实现元素 B，而 B implements 意图元素 A；在这种情况下，C 通过实现链间接承载 A。你的任务是让这种实现链条可追踪、可解释，而不是强制所有节点都直接挂到意图层。',
        ...
        '   - 非显性测试默认物理放在对应实现元素目录下的 tests/ 中；跨目录测试默认放在最近公共祖先目录下，并在相关 README 中回填归属',
        '11. OVERALL.md 与 README.md 的契约格式必须统一采用共享骨架，但根契约与元素契约承担不同字段职责。根级总入口由 OVERALL.md 唯一承载；子目录局部契约默认由 README.md 承载。README.md 可以引用 OVERALL.md，但不得重复定义根级规则。',
        ...
        '   - 契约落盘结果：说明你已更新 OVERALL.md 与哪些 README.md，并概述关键规则、关键元素与局部契约',
