...
8. Treat `argo-systemarchitecture` as the intent graph write boundary: preview the mutation, inspect schema/ArchiMate errors, apply only on a clean preview, then run `node .github/validator/script/validateSystemArchitecture.js` as the post-write confirmation.
