---
name: argo-init
description: "检查 ARGO MCP 是否正常，并完成 NEO4J 初始同步。Use when the user asks to verify Argo MCP readiness and perform or verify the canonical JSON-to-Neo4j initial sync. Keywords: ARGO INIT, harness init, MCP health check, Neo4j initial sync."
...
`argo-init` 现在只负责两件事：检查 `argo` MCP 是否正常，以及完成或验证 canonical intent graph 的 Neo4j 初始同步。它不再负责调用旧的工作区 bootstrap / `initializeWorkspace` 工具。
...
- **MUST** 报告 `argo` MCP 是否通过、Neo4j 是否通过、以及初始同步是否完成。
