...
async function main() {
    ...
    const suitePath = path.join(__dirname, '..', 'suite', 'intentGraphSearchExpansionExtensionHost.js');
    ...
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), 'argo-intent-graph-search-'));
    const workspacePath = path.join(tempRoot, 'IntentGraphSearchWorkspace');
    ...
    try {
        try {
            ...
            if (process.platform === 'win32' && isWindowsUpdateMutexFailure(error)) {
                console.warn('Skipping intent-graph search expansion testcase because the local VS Code runtime is locked by an in-progress Windows update.');
                return;
            }
