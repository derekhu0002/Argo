...
- Source provenance comes only from adapter-recorded source kind/path/key/read operation/alias-chain traces. Fixtures may alter adapter behavior but cannot submit a `direct`, `alias`, `fallback`, or `indirect` verdict.
