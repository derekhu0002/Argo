...
- `dependency-direction.guard.js` owns no command validator. It permits `child_process` only in `systemMetadataCommandAdapter.js`, then calls that production module's callback-scoped test composition so all safe and bypass fixtures traverse the same private production validator and zero-argument adapter. Four safe templates execute; extra capability arguments, shell, dynamic/concatenated argv, flags/path injection, Python/PowerShell/cmd/Node sidecars, arbitrary/network executables, stdin, and secret-bearing argv/environment fail exactly `SYSTEM_METADATA_COMMAND_PROHIBITED` with zero executor calls. Every other production Graph RAG module remains process-free.
