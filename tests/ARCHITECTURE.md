...
For the W7 handoff, `tests/architecture/production-graph-rag/architecture-boundary.guard.js`, `tests/architecture/production-graph-rag/dependency-direction.guard.js`, `tests/architecture/production-graph-rag/explicit-entrypoint-correctness.guard.js`, and `tests/architecture/production-graph-rag/implementation-traceability.guard.js` are the critical non-explicit guards. They protect the W7 quality benchmark boundary, delivery sequence gate, frozen DT-18/TS-08 entrypoint assertions, and direct mappings to `grag-quality-gate` and `grag-seven-wave-delivery`.
