...
- Frozen source fixtures cover process-only, file-only, matching/conflicting dual sources, missing/blank/duplicate/unknown keys, root/alternate/tracked/reparse files, ACL failure, CLI/fallback/alias/indirect sources, and Windows ACL execution without using the real `.argo/.env`.
