...
- The process-only loader guard propagates source provenance through assignments and property access. It allows `.env`/JSON parsing only for the five approved non-sensitive fields and allows only direct `process.env.QWEN_KEY`; frozen safe/bypass fixtures reject parsed/file/config/alias/fallback/destructured/indirect secret sources without reading a secret value.
