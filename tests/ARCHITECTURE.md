...
- DT-19 uses `productionGraphRagHarness.js` to assert capacity evidence without exposing runtime composition plumbing. It freezes the real chain from `evaluatePhase1QualityBenchmark(request)` into `evaluateCapacityEvidence(request)`, requires actual DT-18 quality output to preserve result ids or result cardinality plus measured precision for every declared purpose, requires missing quality evidence to block clearly, and rejects any silent cap, budget, pagination, truncation, continuation, top-k, token-budget, result-limit, or capacity-policy decision field.
...
- `tests/explicit/entries/runCapacityEvidence.js` — `ExplicitAcceptanceTestcase-DT-05-R2-DT-19` capacity evidence recording. The control point is the W7 phase-1 capacity evidence request chained from real DT-18 quality evidence. The observation point is one capacity evidence record for every declared purpose, integer non-negative result cardinality derived from actual result ids/cardinality, measured precision inside `[0, 1]`, explicit blocking for missing quality evidence, and absence of cap, budget, pagination, truncation, continuation, top-k, token-budget, result-limit, or capacity-policy decision fields.
