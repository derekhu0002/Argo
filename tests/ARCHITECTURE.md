...
- Source provenance comes only from adapter-issued frozen `{ value, trace }` process reads and `{ key, value, trace }` file reads. Traces contain source kind/path/key/read operation/frozen alias chain and are authenticated by the adapter's identity-based `isIssuedTrace` capability. Fixtures may alter adapter read behavior but cannot submit a `direct`, `alias`, `fallback`, or `indirect` verdict.
- A frozen interface self-test proves direct process and five prohibited reads can return the identical value while exposing distinguishable trusted traces. Accepted fixtures require every trace to be frozen, schema-valid, consumed, and trust-checked; forged, mutable, malformed, or unvalidated traces cannot authorize configuration.
...
- The auth probe preserves raw exceptions before classification and recursively scans message/stack/causes plus sanitized errors, all injected logger events, stdout/stderr, auth/driver/query calls, graph/persistence state, and recursive artifacts. A channel-completeness self-test inserts a canary into each channel and must detect every insertion; the real probe must report zero leaks and zero persistence.
