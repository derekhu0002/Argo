...
- `dependency-direction.guard.js` permits `child_process` only in `systemMetadataCommandAdapter.js` and dynamically records its four zero-argument capabilities. Safe fixtures cover the exact two Git, `whoami`, and `icacls` templates; bypass fixtures reject shell execution, extra flags, path injection, Python/PowerShell/cmd/Node sidecars, arbitrary/network executables, stdin, and secret-bearing argv/environment. Every other production Graph RAG module remains process-free.
