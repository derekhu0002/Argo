...
- W6 DT-00-W6, DT-13, DT-14, and DT-15 use `intentArchitectureQueryHarness.js` to assert coherent-result version evidence, same-version endpoint closure with structural error channels, complete non-cascading View closure, and exactly one ordered first-inclusion reason with non-overwriting policy/index/version evidence. Entrypoints preserve GIVEN / WHEN / THEN readability and do not parse low-level MCP, filesystem, or environment plumbing.
...
- `tests/explicit/entries/runCoherentIntentReading.js` — `ExplicitAcceptanceTestcase-DT-00-W6-CoherentResultRegression` coherent result version evidence. The control point is legacy and semantic-query tasks against the same canonical graph version. The observation point is complete legacy reading plus semantic `canonicalVersion` evidence; missing version evidence fails `DT00_CANONICAL_VERSION_MISSING`.
- `tests/explicit/entries/runRelationshipEndpointClosure.js` — `ExplicitAcceptanceTestcase-DT-13` same-version relationship endpoint closure. The control point is a purpose-closed relationship range. The observation point is relationship objects with source/target Element objects from the same canonical version, plus explicit dangling and cross-version structural errors.
- `tests/explicit/entries/runCompleteViewClosure.js` — `ExplicitAcceptanceTestcase-DT-14` complete non-cascading View closure. The control point is a matched View sharing members with overlapping Views. The observation point is complete metadata, viewpoint binding, member ids/objects, relationship ids/objects, endpoints, and no overlapping-View cascade.
- `tests/explicit/entries/runFirstInclusionProvenance.js` — `ExplicitAcceptanceTestcase-DT-15` single ordered first-inclusion provenance. The control point is duplicate discovery through semantic seed, endpoint closure, purpose policy, and complete View closure. The observation point is exactly one `firstInclusionReason`, non-overwriting supplementary reasons, and purpose/policy/canonical/content/index/alignment evidence.
