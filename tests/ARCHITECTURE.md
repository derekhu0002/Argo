...
- W7 DT-18 and TS-08 use `productionGraphRagHarness.js` to assert business benchmark and delivery-gate semantics without exposing runtime composition plumbing. DT-18 freezes the approved five-purpose benchmark id, mandatory key seed ids, recalled key seed ids, missing-key-seed emptiness, expected closure correctness, unrelated-query zero forced hits, recorded precision per purpose and aggregate, and absence of an invented release precision threshold. TS-08 freezes prerequisite blocking for W2-W6, DT-18-required blocking after prerequisites are present, and overall-delivery permission only after the W7 quality benchmark passes.
...
For the W7 handoff, `tests/architecture/production-graph-rag/architecture-boundary.guard.js`, `tests/architecture/production-graph-rag/dependency-direction.guard.js`, `tests/architecture/production-graph-rag/explicit-entrypoint-correctness.guard.js`, `tests/architecture/production-graph-rag/implementation-traceability.guard.js`, and `tests/architecture/production-graph-rag/coding-scope-authorization.guard.js` are the critical non-explicit guards. They protect the W7 quality benchmark boundary, delivery sequence gate, frozen DT-18/TS-08 entrypoint assertions, direct mappings to `grag-quality-gate` and `grag-seven-wave-delivery`, W7-only authorization for `evaluateDeliverySequence(request)`, DT-19 capacity evidence exclusion, and runner-owned `deliveryStatus` authority.
