...
- ACL fixtures cover current-identity allow/deny, inherited broad allow, broad deny-only, and unverifiable output using principal-bound `icacls` ACE lines.
...
- A recording Neo4j adapter proves the database password reaches only `neo4j.auth.basic`; authentication failure produces no query and all recorded Cypher text/parameters remain canary-free.
