#!/usr/bin/env python3
"""
Screenshot Analyzer — verify HarmonyOS screenshots via pixel analysis.

Usage:
    python screenshot-analyzer.py <screenshot.jpeg> [options]
...
def check_card_diversity(img, region_tuple):
    """Check if a card region has diverse colors (>30 unique with stride=5)."""
    region = img.crop(region_tuple)
    colors = set()
    for y in range(0, region.height, 5):
        for x in range(0, region.width, 5):
            colors.add(region.getpixel((x, y))[:3])
    return len(colors), len(colors) > 30
...
def main():
    ...
    while i < len(args):
        ...
        if arg == '--summary':
            ...
            i += 1
            threshold = 100
            # Check if next arg is a number (threshold)
            if i < len(args) and args[i].isdigit():
                threshold = int(args[i])
            else:
                i -= 1  # No threshold provided, use default
        ...
        elif arg == '--card-diversity':
            i += 1
            parts = args[i].split(',')
            region_tuple = tuple(int(p.strip()) for p in parts)
            count, is_diverse = check_card_diversity(img, region_tuple)
            if is_diverse:
                print(f"PASS: Card region {region_tuple}: {count} unique colors (diverse, real photo)")
                checks_passed += 1
            else:
                print(f"FAIL: Card region {region_tuple}: only {count} unique colors (not diverse)")
                checks_failed += 1
    ...
    # If no checks run, just print image info
