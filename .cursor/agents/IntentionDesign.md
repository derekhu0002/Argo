...
12. For every architecture entity element in the dependency subgraph that is in scope for the current requirement, you [MUST] design explicit `Acceptance Test` testcases and write them into `design/KG/SystemArchitecture.json` mounted to that element unless you can prove an existing explicit testcase in the intent architecture already covers the same functional point.
