...
## Orchestrator Coordination Boundary And Automatic Delegation

Orchestrator dispatches stages and audits only; it does not manage child-level stage-internal work. Stage owners own automatic delegation and internal delegation inside their stages. Orchestrator observes validated stage summaries and bounded summaries, not raw child evidence, and no stage is bypassed.

### Preserved gates
Preserve stage dispatch, stage commits, approvals, approval forwarding, handoff validation, audit routing, Delivery Status Governance, Stage Commit Governance, delivery status checks, and final workflow closure. Existing gates that require human partner approved handoffs, `validateStageHandoff`, and `runArchitectureTests` remain mandatory.

### Shared vocabulary acknowledged by Orchestrator (stage-owned execution)
Orchestrator does not execute child-level work, but stage Agents must follow the shared automatic delegation vocabulary: atomic local work stays local; large work returns bounded structured evidence under final accountability of the stage owner; hard triggers include G above 10 with a slice plan / independently verifiable slice, two independently decidable hypotheses delegated separately with parent synthesizes, two non-lightweight evidence channels with verifier and channel gatherers producing a singular verdict, dependency-independent disjoint authorized write sets that may run concurrently with exact write sets, and broad unknown-repository / open-internet bounded exploration returning structured findings and evidence locations; prohibitions cover atomic local work, shared-write, negative-value, reserved final gates with no child and one prohibition reason; resource limits require that simple work uses one child level where sufficient, stage owner to verifier to gatherer paths stay within two child edges with no third child edge, four active children max, eligible queued work fills released slots, dependency-blocked work does not consume an active slot, and overflow queues by dependency, risk, and blocking impact; returns include identity, verdict, decisive evidence, missing channels, conflicts, change results, next action, strongest 3-5 ordinary supports, decisive counterexample, externally addressable evidence locations, without raw logs and without full search process; dispositions are exactly one of one same-session retry, supplement missing evidence, serialize write conflict, or escalate authority; behavior proxy acceptance requires every hard-trigger decision is traceable, atomic tasks do not delegate, bounded summaries for depth / concurrency / retry, and existing gates pass. Do not claim token-reduction telemetry.
