...
- **MUST** delegate decision-tree-to-architecture integration to at least 5 dedicated `TaskTidyGraphIntegrator` subagents; the `task-tidy` host agent keeps orchestration, validation, conflict resolution, and synthesis responsibility.
...
- **MUST** ensure each `TaskTidyGraphIntegrator` subagent uses a different LLM/model from the others when the runtime supports model selection.
- **MUST** block completion if fewer than 5 distinct integration LLMs are available; do not replace heterogeneous integration with repeated same-model attempts.
- **MUST** return to the same `TaskTidyGraphIntegrator` subagent sessions and continue the integration work when host validation finds an architecture-mapping omission, architecture conflict, unreasonable graph placement, missing testcase/control/observation mapping, or untraceable decision-tree node; do not start new sessions merely to fix discovered gaps.
...
- 从 Business Partner 对话中重建至少三层决策树：问题定义、分解维度、分支、推荐答案、人类确认/否决、依赖前提、风险和验收边界。
...
- 委托至少 5 个 `TaskTidyGraphIntegrator` 子 agent 分别执行主整理候选：读取 `DecisionTreeRecord`、当前 `SystemArchitecture.json`、相关仓库证据和 MCP schema 约束，产出 mutation 计划、覆盖矩阵和 integration readiness report。
- 每个 `TaskTidyGraphIntegrator` 都是 integrator，不是 validator；它负责提出完整整合方案，但最终验收由 `task-tidy` host agent 完成。
...
- `task-tidy` host agent 必须综合至少 5 个 `TaskTidyGraphIntegrator` integration reports。
...
- 任一候选方案或综合结果暴露整合遗漏、冲突、不合理落点或不可追踪节点时，host agent 必须 resume 同一批 `TaskTidyGraphIntegrator` 子 agent 会话继续修正，并在这些原会话中补齐缺口；不得为了修复已发现的问题另开新 session。
...
- 至少 5 个异构 `TaskTidyGraphIntegrator` 子 agent 的模型/LLM 标识、整合候选摘要和 integration blockers。
- `task-tidy` host agent 对所有整合候选的最终验收结论和综合判断：accepted、needs reintegration、blocked。
- 如果 blocked，说明是 LLM 多样性不足、图谱 mutation 阻塞，还是决策树字段不足导致无法映射。
