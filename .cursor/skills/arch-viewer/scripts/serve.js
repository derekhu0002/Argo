...
function findRoot() {
  ...
  for (let i = 0; i < MAX_DEPTH; i++) {
    ...
    if (parent === dir) break;
    dir = parent;
    if (fs.existsSync(path.join(dir, 'schema', 'SystemArchitecture.schema.json'))) return dir;
    if (fs.existsSync(path.join(dir, 'package.json')) && fs.existsSync(path.join(dir, '.cursor'))) return dir;
  }
  return process.cwd();
}
...
const PATHS = {
  ...
  schema: path.join(ROOT, 'schema', 'SystemArchitecture.schema.json'),
