...
当验收范围涉及 `design/KG/SystemArchitecture.json` 的变更时，必须额外检查该变更是否通过 `argo-systemarchitecture` MCP mutation gateway 完成，并且是否有成功的 preview/apply 与 `validator_validateSystemArchitecture` 结果。只有最终 JSON 通过 validator 但缺少 MCP 写入证据时，应判定为治理流程缺口。