...
*   **意图图谱写入边界[MUST]**：本 skill 只负责业务决策树、推荐答案和验收标准。任何对 `design/KG/SystemArchitecture.json` 的新增、修改、删除，都必须交给 Intent Design 并通过 `argo-systemarchitecture` MCP mutation gateway 的 preview/apply 流程完成；严禁直接编辑 JSON。
