...
25. WP-P1 production semantic persistence is a separate durable path from live-E2E evidence. Production records use stable canonical identity with complete channel, canonical/content/index, provider/model/model-version/dimensions, and vector metadata; changed records upsert and tombstones delete without production `runId` or cleanup behavior.
26. WP-P1 full backfill begins only after structural projection completes for the same canonical version, independently enumerates every Element, ArchitectureRelationship, and View in bounded checkpointed batches, isolates failures, resumes without restarting completed work, reruns idempotently, performs no fake canonical mutation, and marks Aligned only after all three channels complete.
27. Canonical JSON remains authoritative throughout WP-P1. Neo4j and durable checkpoints remain subordinate projection/index state; existing live-E2E runId cleanup stays test-only and unchanged.
...
| Production Semantic Persistence Boundary | `.argo/scripts/graph-rag/semantic-persistence/` | Own the separate durable production semantic projection and explicit bounded, resumable, idempotent three-channel backfill after structural projection. | `createProductionSemanticProjectionStore(dependencies)` and `createProductionSemanticBackfill(dependencies).execute({ explicitOptIn })` |
