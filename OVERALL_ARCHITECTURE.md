...
21. W6 structural closure begins after W5 purpose-policy range selection and is complete only when semantic results expose governing canonical version evidence equal to the legacy graph version, every returned relationship has source/target ids plus both same-version endpoint Elements or explicit structural errors, matched Views return complete metadata, parent viewpoint, exact member/relationship object sets, and endpoints without overlapping-View cascade, and every returned object has exactly one ordered first-inclusion reason plus non-overwriting supplementary reasons, policy id, policy parameters/anchors, and index/version evidence.
