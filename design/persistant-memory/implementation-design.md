...
- DT-06/DT-07 now retain deterministic policy evidence for the four semantic categories (`intent-decision`, `implementation-design`, `coding-repair`, and `audit`). DT-12 owns the anchored graph-tidy compatibility control and proves canonical snapshot equality with no additional semantic-boundary invocation.
...
- The intent-query correctness guard rejects graph-tidy in the semantic closure category list, freezes an explicit anchor in DT-12, and requires the compatibility Harness snapshot path to avoid MCP semantic dispatch.
