...
- The correction handoff narrows CodingAndReparing authorization to `.argo/scripts/systemarchitecture-mcp-server.js`: remove every content-, lifecycle-, subject-, control-word-, and anchor-based deterministic selector. Only no-argument and graph-tidy full-snapshot bypasses remain; deterministic compatibility is available only through explicit third-argument test dependencies.
