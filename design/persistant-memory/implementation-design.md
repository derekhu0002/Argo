...
- System metadata is a dedicated inward boundary rather than a Python dependency: only `systemMetadataCommandAdapter.js` may import Node `child_process`, and it exposes four zero-argument capabilities over fixed `git check-ignore`, `git ls-files`, `whoami`, and `icacls` templates. Frozen safe/bypass fixtures prohibit shell, command/flag/path variation, sidecars, arbitrary/network executables, stdin, and secret-bearing argv/environment.
