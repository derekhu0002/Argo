...
- Source adapters now return immutable value/trace envelopes directly to the resolver: process reads return `{ value, trace }`, file reads return `{ key, value, trace }`, and `isIssuedTrace(trace)` authenticates object identity. Frozen self-tests prove identical values carry distinguishable direct/CLI/literal/fallback/alias/indirect traces and cloned, mutable, malformed, or unchecked traces cannot authorize configuration.
