...
flowchart TD
    ...
    G --> P[人类伙伴按顺序提交 Orchestrator 迭代交付]
...
问题处理也必须先经过意图设计，因为“问题”不一定是代码 BUG。它可能来自需求理解错误、验收边界遗漏、意图图谱表达不准确，也可能来自实现架构契约、测试入口或编码实现偏差。先由 `IntentionDesign` 判断问题是否属于意图架构问题；如果是，应转入新需求开发流程，通过 `business-partner` 重新整理需求与验收边界，再通过 `task-tidy` 形成任务，由人类伙伴按顺序提交给 `Orchestrator` 迭代交付。若意图正确，再由 `ImplementationDesign` 判断实现架构是否需要调整；只有确认问题落在代码实现层，才交给 `CodingAndReparing` 直接修复。
