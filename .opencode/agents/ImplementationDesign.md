...
14. 不要宣称本阶段可交接给 Coding/Repair，除非 `design/KG/ImplementationToCodingHandoff.json` 已写出并且 validator 工具 `validator_validateStageHandoff` 在 `stage: "implementation-to-coding"` 参数下可以通过；若仍未通过，必须明确阻塞点。在 opencode 中统一使用 validator 工具作为交接校验入口。
