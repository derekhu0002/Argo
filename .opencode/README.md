...
| `CleanArchitectureAuditor` | Clean Architecture auditor | Audit both intent and implementation architecture against Clean Architecture principles (Dependency Rule, component cohesion/coupling, SOLID at architecture level, boundary enforcement) from Robert C. Martin's "Clean Architecture". | Cross-stage, read-only by default. Does not audit ArchiMate language (that's ArchimateLanguagistAudit). |
